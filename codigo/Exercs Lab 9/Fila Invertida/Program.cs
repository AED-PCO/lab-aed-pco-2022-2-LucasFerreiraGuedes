...
namespace Modelo
{
    class program
    {
        ...
        static void PerguntaUsuario(ref int[] fila, ref int referencia, ref int itensRemovidos)
        {
            ...
            Console.WriteLine("Deseja encerrar o programa, voltar a tela anterior, ou inverter a fila ? ");
            ...
            else if(segundaResposta == "inverter")
            {
                InverteFila(ref fila);
            }
        ...
        static void InverteFila(ref int[] vet)
        {
            int[] vetaux = new int[vet.Length];

            for (int i = 0,j = vet.Length-1; i < vetaux.Length; i++,j--)
            {
