...
namespace Modelo
{
    class program
    {
        ...
        static void PerguntaUsuario(ref int[] pilha, ref int referencia, ref int itensRemovidos)
        {
            ...
            Console.WriteLine("Deseja encerrar o programa,voltar a tela anterior ou mostrar a pilha copiada? ");
            ...
            else if (segundaResposta == "pilha")
            {
                Console.Clear();
                CopiarPilha(ref pilha, ref itensRemovidos);
            }
        }
        ...
        static void CopiarPilha(ref int[] pilha,ref int itensRemovidos)
        {
            int[] aux = new int [pilha.Length];
            int[] pilhaCopiada = new int[pilha.Length];

            Console.WriteLine($"Você possui {pilha.Length} valores, todos deverão ser retirados: ");
            for(int i = 0,j = pilha.Length-1;i < pilha.Length; i++,j--)
            {
                aux[i] = pilha[j];
            }
            //imprimeVetor (aux);
